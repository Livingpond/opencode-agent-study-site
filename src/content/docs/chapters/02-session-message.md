...
用户输入与会话模块负责把外部请求变成稳定的内部事实：一个 session 下的 user message 和一组 message parts。来源：<details class="source-ref source-ref--inline">
  <summary>
...
CLI/API 只提供输入，agent loop 只消费消息历史。中间的桥就是 session/message 层：它把文本、文件、agent mention、MCP resource、权限覆盖、模型选择等都整理成统一结构。来源：<details class="source-ref source-ref--inline">
  <summary>
...
<span class="source-line"><span class="source-line-number">67</span><span class="source-line-text">export const CommandPayload = Schema.Struct(Struct.omit(SessionPrompt.CommandInput.fields, [&quot;sessionID&quot;]))</span></span>
<span class="source-line"><span class="source-line-number">68</span><span class="source-line-text">export const ShellPayload = Schema.Struct(Struct.omit(SessionPrompt.ShellInput.fields, [&quot;sessionID&quot;]))</span></span></code></pre>
</details>：用 `Struct.omit` 从 `SessionPrompt` 输入类型派生 API payload。
...
</details>：定义 `session.prompt` endpoint。
...
</details>：handler 调用 `promptSvc.prompt`。
...
</details>：解析 file/resource/agent parts。
...
</details>：写入 message/parts 并发布 prompt event。
...
`nextPrompt` 会把 text/file/agent/reference/synthetic 分组，并在 experimental event system 打开时发布事件。来源：<details class="source-ref source-ref--inline">
  <summary>
    <span class="source-ref-title">packages/opencode/src/session/prompt.ts</span>
