...
<!-- source-ref path="packages/opencode/package.json" lines="21-23" title="package bin：opencode 命令指向可执行入口" note="安装后用户敲 opencode，先进入这个 package 暴露的 bin，再由启动脚本加载 TypeScript 入口。" -->

<!-- source-ref path="packages/opencode/src/index.ts" lines="58-110" title="index.ts：从 process.argv 到 yargs 全局中间件" note="这里是 CLI main 的核心启动段：拿 argv、创建 yargs、注册全局 option，并初始化日志和进程环境变量。" -->

<!-- source-ref path="packages/opencode/src/index.ts" lines="158-180" title="index.ts：注册 RunCommand、ServeCommand 等子命令" note="这段决定 opencode 这个根命令下面能接哪些子命令，RunCommand 就是在这里挂进去的。" -->

<!-- source-ref path="packages/opencode/src/cli/effect-cmd.ts" lines="70-93" title="effectCmd：给每个 CLI handler 套运行时外壳" note="它统一处理 InstanceContext 加载、Effect runtime 执行和 finally 清理，让具体命令只写业务逻辑。" -->

<!-- source-ref path="packages/opencode/src/cli/cmd/run.ts" lines="127-245" title="RunCommand：定义 run [message..] 和参数面" note="这段展示 run 命令为什么不是简单字符串输入，而是包含 session、model、agent、file、attach、interactive 等完整入口。" -->

<!-- source-ref path="packages/opencode/src/cli/cmd/run.ts" lines="334-358" title="RunCommand：解析 --file 和 piped stdin" note="用户输入会被拆成 file parts 和 text part，后面一起交给 session prompt。" -->

<!-- source-ref path="packages/opencode/src/cli/cmd/run.ts" lines="396-516" title="RunCommand：创建、继续和 fork session" note="coding agent 需要可恢复上下文，所以 CLI 在真正 prompt 前会先确定 session。" -->

<!-- source-ref path="packages/opencode/src/cli/cmd/run.ts" lines="637-759" title="RunCommand：事件订阅 loop 如何渲染输出" note="模型回复、tool 执行、错误、权限请求都会变成事件，CLI 只是订阅并渲染这些事件。" -->

<!-- source-ref path="packages/opencode/src/cli/cmd/run.ts" lines="768-879" title="RunCommand：调用 session prompt，并在本地模式复用 Server.Default" note="非交互模式订阅事件后调用 client.session.prompt；本地模式通过 in-process fetch 进入同一套 server handler。" -->
